...
for fidx in range(len(fits)):
  ...
  if _fit.split(":")[0] == "bestfit":
    ...
    impactcmd1 = "cd runImpacts%s_%s; source /cvmfs/cms.cern.ch/crab3/crab.sh; combineTool.py --task-name %s_initialFit -M Impacts -m 125 %s %s --doInitialFit --robustFit 1 -n _%s_initialFit %s %s %s"%(opt.ext,opt.mode,_name,d_opts,exp_opts,_name,_fit_opts,pdf_opts,common_opts)
    impactcmd2 = "cd runImpacts%s_%s; source /cvmfs/cms.cern.ch/crab3/crab.sh; combineTool.py --task-name %s_doFits -M Impacts -m 125 %s %s --doFits -n _%s_initialFit %s %s %s"%(opt.ext,opt.mode,_name,d_opts,exp_opts,_name,_fit_opts,pdf_opts,common_opts)
    if opt.batch != 'local':
