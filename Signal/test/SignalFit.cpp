  ...
  desc1.add_options()
   ...
   // ("nCats,n", po::value<int>(&nCats_)->default_value(9),                                    			"Number of total categories")
    ...
    ("verbose,v", po::value<int>(&verbose_)->default_value(0),                                			"Verbosity level: 0 (lowest) - 3 (highest)")
		("useMerged",	po::value<bool>(&useMerged_)->default_value(false),														"Skip the fitting and package a merged file")
		...
		("split",	po::value<string>(&splitStr_)->default_value(""),														"handle single cat/procs at a time, specify as proc,cat  (default false)")
...
int main(int argc, char *argv[]){
    ...
    inWS = (RooWorkspace*)inFile->Get("wsig_13TeV");
    } else {
  ...
  if (!useMerged_){
	while (datfile.good()){
		string line;
    std::cout << " LINE DATFILE " << line << std::endl;
    ...
    if (split_.size()==2){
      ...
      std::cout << " proc " << proc << ", splitProc " << splitProc << ",  proc.compare(splitProc)  "<<  proc.compare(splitProc) << std::endl; 
      if ( proc.compare(splitProc) == 0 ) {
        std::cout << " -- flashggCats_[cat] " << flashggCats_[cat]  << ", splitCat " << splitCat<< ", flashggCats_[cat].compare(splitCat) " << flashggCats_[cat].compare(splitCat) <<  std::endl; 
        if ( flashggCats_[cat].compare(splitCat) == 0 ) {
          continueFlag =0; 
        }
      }
    }
    ...
    if(continueFlag) {
      std::cout << " skipping "<< Form("[INFO] Running fits for proc:%s - cat:%d with nGausRV:%d nGausWV:%d",proc.c_str(),cat,nGaussiansRV,nGaussiansWV) << endl;
      continue;
    }
    cout << "-----------------------------------------------------------------" << endl;
    ...
    for (int mh=mhLow_; mh<=mhHigh_; mh+=5){
      ...
      if (isFlashgg_){
        ...
        outWS->import(*data0,RecycleConflictNodes());
        ...
        if (pdfWeights_){
          ...
          for (int ipdf =0 ;ipdf< pdfWeights_ ; ipdf++){
            ...
            outWS->import(*dataH,RecycleConflictNodes());
          }
        ...
        if (changeIntLumi){
          ...
          data = (RooDataSet*) data0->reduce(RooArgSet(*mass, *dZ, *weight0))->Clone();
    ...
    if (!cloneFits_) {
      ...
      std::cout << "DEBUG 111215 - Set Dtasets RV : "<< std ::endl;
      typedef std::map<int,RooDataSet*>::iterator it_type;
      for(it_type iterator = datasetsRV.begin(); iterator != datasetsRV.end(); iterator++) {

      std::cout << "DEBUG 111215 - ---------------> " << iterator->first << ", "<< *(iterator->second)<< std ::endl;
    ...
    if (!runInitialFitsOnly_) {
      ...
      if (isFlashgg_){
        ...
        outWS->import(*intLumi,RecycleConflictNodes());
  ...
  if (!runInitialFitsOnly_) { 
    ...
    Packager packager(inWS,procs_,nCats_,mhLow_,mhHigh_,skipMasses_,sqrts_,skipPlots_,plotDir_,mergeWS,cats_,flashggCats_);
    packager.packageOutput();
    }
    sw.Stop();
