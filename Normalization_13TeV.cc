...
// Modifying this class to cater for 13TeV values.
// Eventually would want a unified class for 7, 8, 13TeV etc..
...
int Normalization_13TeV::Init(int sqrtS){
	...
	// get cmssw base, and path to signal directory.
	const char * env = gSystem->Getenv("CMSSW_BASE");
	std::string flashggRt = env;
	if (flashggRt !=0){
		flashggRt = flashggRt+"/src/flashggFinalFit/Signal";
	}
	// use python to load the buildSMHiggsXSBR tool
	if( ! TPython::Exec(Form("buildSMHiggsSignalXSBR = imp.load_source('*', '%s/python/buildSMHiggsSignalXSBR.py')",flashggRt.c_str())) ) {
		std::cout<<  "Importing buildSMHiggsSignalXSBR from python failed. exit." << std::endl;
		return 0;
	}
	// FIXME th buildSMHiggsSignalXSBR tool needs to be amended for 13TeV and beyond
	TPython::Eval(Form("buildSMHiggsSignalXSBR.Init%dTeV()", sqrtS));
	
	// FIXME can we now get away with fewer mass points?
...
// What does this function do ? LC FIXME
void Normalization_13TeV::FillSignalTypes(){
...
TGraph * Normalization_13TeV::GetSigmaGraph(TString process)
{
	...
	if ( process == "ggh" || process == "ggH") { // dont want to hard code this if possible LC FIXME
		XSectionMap = &XSectionMap_ggh;
	} else if ( process == "vbf" || process == "VBF" ) { // FIXME
		XSectionMap = &XSectionMap_vbf;
	} else if ( process == "vbfold") { // what is this ? FIXME
...
double Normalization_13TeV::GetVBFCorrection(double mass) {
	return GetXsection(mass,"vbf")/GetXsection(mass,"vbfold");
}

// Simple accessors
TString Normalization_13TeV::GetProcess(int ty){
...
double Normalization_13TeV::GetMass(int ty){
	if (ty < -7999){  // We dont go below 80 GeV and Spin samples in the 100 range
		return double(-ty/1000);
	}	 
	else return SignalTypeMap[ty].second;
}
//// double Normalization_13TeV::GetXsection(int ty){
...
////   }
//// }
//// double Normalization_13TeV::GetBR(int ty){
////   std::pair<TString,double> proc_mass = SignalTypeMap[ty];
...
void Normalization_13TeV::CheckNorm(double Min, double Max, double Step, TString histname="") {
...
void Normalization_13TeV::PlotXS(double Min, double Max){
	...
	else	     can->SaveAs("xsections_13TeV.pdf");
}
void Normalization_13TeV::PlotBR(double Min, double Max){
...
void Normalization_13TeV::PlotExpected(double Min, double Max){
	...
	else		can->SaveAs("signalnormalization_13TeV.pdf");
}
