...
for gr in grs_ratio.itervalues():
...
def extractLabelStr(p,res,theory):
  ...
  if x >= 9.95: lstr += "%.0f^{+%.0f}_{-%.0f}"%(x,abs(u_up),abs(u_down))
  elif x >= 1.: lstr += "%.1f^{+%.1f}_{-%.1f}"%(x,abs(u_up),abs(u_down))
  else: lstr += "%.2f^{+%.2f}_{-%.2f}"%(x,abs(u_up),abs(u_down))
  return lstr 
...
haxes.GetYaxis().SetTitle("#sigma#upointBR [fb]")
...
leg.AddEntry(gr_theory_dummy,"Uncertainty on SM pred.","F")
leg.Draw("SAME")
# Add box for values
...
lat2.DrawLatex(0.13,0.32,"H#rightarrow#gamma#gamma, |y_{H}| < 2.5")
if "minimal" in opt.mode: lat2.DrawLatex(0.13,0.25,"STXS stage 1.2: minimal")
else: lat2.DrawLatex(0.13,0.25,"STXS stage 1.2: maximal")
...
else: haxes_r.GetXaxis().SetLabelSize(0.06*padSizeRatio)
...
for i in range(1,nPOIs):
  ...
  vlines['vline_r_%g'%i].SetLineColorAlpha(ROOT.kGray,0.5)
...
hatchBox.Draw("SAME")
hlines = {}
yvals = [0.5,0.75,1,1.25,1.5,2.,3.,4.,5.]
for i in range(len(yvals)):
  yval = yvals[i]
  hlines['hline_%g'%i] = ROOT.TLine(0,yval,nPOIs,yval)
...
if "obs" in opt.inputJson:
  canv.SaveAs("/eos/home-j/jlangfor/www/CMS/hgg/stxs_runII/May20/pass0/checks/stxs_summary_%s_obs.pdf"%opt.mode)
  canv.SaveAs("/eos/home-j/jlangfor/www/CMS/hgg/stxs_runII/May20/pass0/checks/stxs_summary_%s_obs.png"%opt.mode)
else:
  canv.SaveAs("/eos/home-j/jlangfor/www/CMS/hgg/stxs_runII/May20/pass0/checks/stxs_summary_%s_exp.pdf"%opt.mode)
  canv.SaveAs("/eos/home-j/jlangfor/www/CMS/hgg/stxs_runII/May20/pass0/checks/stxs_summary_%s_exp.png"%opt.mode)
