...
#  flashggSysts['phoIdMva'] = 'n_id_mva'
#  flashggSysts['regSig'] = 'n_sigmae'
if options.isBinnedSignal:
	#flashggSysts['E_scale'] = 'n_e_scale'
	flashggSysts['MCScale'] = 'n_e_scale'
	#flashggSysts['E_res'] = 'n_e_res'
	#flashggSysts['E_res'] = 'n_e_res'
if options.isSpinModel:
...
#	else:
	...
	outFile.write('Auto-generated by h2gflashgg/Macros/makeParametricModelDatacard.py\n')
		...
		for p in options.procs:
			...
			if p in bkgProcs:
				...
				if options.isBinnedSignal:
					...
					if options.isSpinModel: 
						scale=1.
						if flashggProc[p] in scaleFactors.keys():
							scale = scaleFactors[flashggProc[p]]
		...
		if 'pdfWeight' and 'QCDscale' in flashggSyst: # special case
			if options.isBinnedSignal: 
				...
				for c in options.cats:
					for p in options.procs:
						if '%s:%s'%(p,c) in options.toSkip: continue
						#if p=='qqH': outFile.write(getGlobeLine(flashggProc[p],c,flashggSyst))
...
def printVbfSysts():
	...
	for p in options.procs:
		...
		for cats in vbfMigrateToCats:
			sum=0
			for c in cats:
				th1f =  inFile.Get('th1f_sig_%s_mass_m125_%s'%(flashggProc[p],c))
				sum += th1f.Integral()
...
def printLepSystsOld():
	...
	for p in options.procs:
		...
		for c in range(options.ncats):
			if '%s:%s'%(p,c) in options.toSkip: continue
			if p in bkgProcs: continue
			th1f = inFile.Get('th1f_sig_%s_mass_m125_cat%s'%(flashggProc[p],c))
